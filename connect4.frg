  ...
  all s: State | {
    all r, c: Int | r != 5 => {
      (s.board[r][c] = Red or s.board[r][c] = Blue) => {
        s.board[add[r,1]][c] = Red or s.board[add[r,1]][c] = Blue} 
    }
  }
}
...
-- alternating players
-- winning is only on the last state
pred traces {
  some init, final: State {
        -- constraints on init state
        // start[init]               --------- this is commented out because we dont necessarily 
                                      -------- want the first state to be the empty board
